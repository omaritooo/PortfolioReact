  ...
  return (
    <div className="flex flex-col font-raleway gap-y-10">
      ...
      <motion.div
        ...
        className="container flex flex-wrap justify-center px-4 py-4 text-red-400 gap-x-8 sm:gap-x-16 w-fit dark:text-white">
        {data.map((el: { name: string; src: string }) => {
          return (
            <motion.div
              ...
              <BaseSvg src={el.src} />
              <span className="text-lg font-light text-white transition duration-200 opacity-0 group-hover:opacity-100 ">
                {el.name}
              </span>
            </motion.div>
          );
